...
docker run -d --network=testnet -p 443:443 -v $REPO_ROOT/nc-sciencemesh:/var/www/html/apps/sciencemesh -e HOST=$EFSS --name=nc1.docker pondersource/dev-stock-nc1-sciencemesh

docker run -d --network=testnet -e MARIADB_ROOT_PASSWORD=eilohtho9oTahsuongeeTh7reedahPo1Ohwi3aek --name=maria1.docker mariadb --transaction-isolation=READ-COMMITTED --binlog-format=ROW --innodb-file-per-table=1 --skip-innodb-read-only-compressed
docker run -d --network=testnet -p 443:443 -v $REPO_ROOT/nc-sciencemesh:/var/www/html/apps/sciencemesh -e HOST=$EFSS --name=nc1.docker pondersource/dev-stock-nextcloud-solid
...
docker container cp /root/fullchain.pem nc1.docker:/tls/$EFSS.crt
docker container cp /root/privkey.pem nc1.docker:/tls/$EFSS.key
docker restart nc1.docker

waitForPort maria1.docker 3306
waitForPort nc1.docker 443

docker exec -e DBHOST=maria1.docker -e USER=einstein -e PASS=relativity  -u www-data nc1.docker sh /nc-init.sh

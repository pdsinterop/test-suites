...
docker container cp /etc/letseocrypt/archive/$EFSS/fullchain1.pem oc1.docker:/tls/oc1.crt
docker container cp /etc/letseocrypt/archive/$EFSS/privkey1.pem oc1.docker:/tls/oc1.key
docker restart oc1.docker
docker exec -it oc1.docker sed -i "13 i\      5 => '$EFSS'," /var/www/html/config/config.php
...
docker exec maria1.docker mariadb -u root -peilohtho9oTahsuongeeTh7reedahPo1Ohwi3aek owncloud -e "insert into oc_appconfig (appid, configkey, configvalue) values ('scieocemesh', 'iopUrl', 'https://$REVA/');"
docker exec maria1.docker mariadb -u root -peilohtho9oTahsuongeeTh7reedahPo1Ohwi3aek owncloud -e "insert into oc_appconfig (appid, configkey, configvalue) values ('scieocemesh', 'revaSharedSecret', 'shared-secret-1');"

echo Now you should be able to lo:g in at https://$EFSS as einstein / relativity.

#!/bin/bash
set -e
docker run -d --network=testnet --name=revaoc2.docker -v /root/reva:/reva -e HOST=revaoc2 revad
...
docker exec -it maria2.docker mariadb -u root -peilohtho9oTahsuongeeTh7reedahPo1Ohwi3aek owncloud -e "insert into oc_appconfig (appid, configkey, configvalue) values ('sciencemesh', 'meshDirectoryUrl', 'https://meshdir.docker/meshdir');"

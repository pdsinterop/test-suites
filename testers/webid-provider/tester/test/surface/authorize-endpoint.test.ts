...
describe("The server's authorize endpoint", () => {
  ...
  test("Giving consent redirects you back to authorize", async () => {
    ...
    expect(fetchResult.status).toEqual(302);
    expect(fetchResult.headers.get('location')).toEqual(`${authorizationEndpoint}?response_type=id_token%20code&display=&scope=openid%20profile%20offline_access&client_id=coolApp2&redirect_uri=http%3A%2F%2Flocalhost%3A3002%2Fredirect&state=84ae2b48-eb1b-4000-8782-ac1cd748aeb0&nonce=&request=`);
  });

  test("Authorize with cookie after consent redirects you back to the app", async () => {
    // This test uses the consented coolApp2
    const fetchResult = await fetch(`${authorizationEndpoint}?response_type=id_token%20code&display=&scope=openid%20profile%20offline_access&client_id=coolApp2&redirect_uri=http%3A%2F%2Flocalhost%3A3002%2Fredirect&state=84ae2b48-eb1b-4000-8782-ac1cd748aeb0&nonce=&request=`, {
      "headers": {
        cookie
      },
      redirect: "manual"
